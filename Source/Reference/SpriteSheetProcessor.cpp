...
bool USpriteSheetProcessor::ProcessSpriteSheet(const FString& TextureName, const FSpriteSheetInfo& SpriteInfo)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("Imported texture size: %dx%d"), ImportedTexture->GetSizeX(), ImportedTexture->GetSizeY());
	...
	{
		UE_LOG(LogTemp, Error, TEXT("Failed to extract sprites from texture: %s"), *TextureName);
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Error, TEXT("Failed to create animations from sprites"));
		return false;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("Successfully processed sprite sheet: %s"), *TextureName);
	UE_LOG(LogTemp, Log, TEXT("Generated %d sprites and %d animations"), ExtractedSprites.Num(), CreatedAnimations.Num());
...
UTexture2D* USpriteSheetProcessor::ImportTexture(const FString& RawAssetPath, const FString& DestinationPath)
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("Source texture file does not exist: %s"), *RawAssetPath);
		return nullptr;
	}
	...
	{
		UE_LOG(LogTemp, Error, TEXT("Failed to load texture file: %s"), *RawAssetPath);
		return nullptr;
	}

	UE_LOG(LogTemp, Warning, TEXT("Raw file data size: %d bytes"), RawFileData.Num());
	...
	{
		UE_LOG(LogTemp, Error, TEXT("Failed to set compressed data in image wrapper"));
		return nullptr;
	}

	int32 Width = ImageWrapper->GetWidth();
	int32 Height = ImageWrapper->GetHeight();
	UE_LOG(LogTemp, Warning, TEXT("Decoded image dimensions: %dx%d"), Width, Height);

	// Extract raw RGBA pixel data
	TArray<uint8> UncompressedRGBA;
	if (!ImageWrapper->GetRaw(ERGBFormat::BGRA, 8, UncompressedRGBA))
	{
		UE_LOG(LogTemp, Error, TEXT("Failed to get raw pixel data from image"));
		return nullptr;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("Successfully imported texture: %s (%dx%d)"), *AssetName, Width, Height);
	return NewTexture;
#else
	UE_LOG(LogTemp, Error, TEXT("Import functionality is only available in editor builds"));
	return nullptr;
#endif
}
...
bool USpriteSheetProcessor::ApplyPaper2DTextureSettings(UTexture2D* Texture)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("Applied Paper2D texture settings to: %s"), *Texture->GetName());
	UE_LOG(LogTemp, Warning, TEXT("Texture size after settings: %dx%d"), Texture->GetSizeX(), Texture->GetSizeY());
	...
	return true;
#else
	UE_LOG(LogTemp, Error, TEXT("Texture setting modification is only available in editor builds"));
	return false;
#endif
}
...
TArray<UPaperSprite*> USpriteSheetProcessor::ExtractSprites(UTexture2D* Texture, const FSpriteSheetInfo& SpriteInfo)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("DEBUG: Actual texture dimensions: %dx%d"), TextureWidth, TextureHeight);
	UE_LOG(LogTemp, Warning, TEXT("DEBUG: Expected grid: %dx%d"), SpriteInfo.Columns, SpriteInfo.Rows);
	UE_LOG(LogTemp, Warning, TEXT("DEBUG: Calculated sprite size: %dx%d"), SpriteWidth, SpriteHeight);
	
	UE_LOG(LogTemp, Log, TEXT("Extracting sprites from %dx%d texture, sprite size: %dx%d"), 
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Unsupported pixel format, assuming 4 bytes per pixel"));
	}
	...
	for (int32 Row = 0; Row < SpriteInfo.Rows; Row++)
	{
		for (int32 Col = 0; Col < SpriteInfo.Columns; Col++)
		{
			...
			if (!SpriteTexture)
			{
				UE_LOG(LogTemp, Error, TEXT("Failed to create sprite texture: %s"), *SpriteName);
				continue;
			}
			...
			if (!SpritePackage)
			{
				UE_LOG(LogTemp, Error, TEXT("Failed to create sprite package: %s"), *PackagePath);
				continue;
			}
			...
			if (!NewSprite)
			{
				UE_LOG(LogTemp, Error, TEXT("Failed to create sprite: %s"), *SpriteName);
				continue;
			}
			...
			{
				UE_LOG(LogTemp, Error, TEXT("Failed to find sprite properties via reflection"));
			}
			...
			UE_LOG(LogTemp, Log, TEXT("Created sprite: %s at (%d, %d) size (%dx%d)"), 
	...
	UE_LOG(LogTemp, Log, TEXT("Extracted %d sprites total"), ExtractedSprites.Num());
#endif
...
TArray<UPaperFlipbook*> USpriteSheetProcessor::CreateAnimations(const TArray<UPaperSprite*>& Sprites, const FSpriteSheetInfo& SpriteInfo)
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("Sprite count mismatch. Expected: %d, Got: %d"), 
	...
	for (int32 Row = 0; Row < SpriteInfo.Rows; Row++)
	{
		...
		if (!FlipbookPackage)
		{
			UE_LOG(LogTemp, Error, TEXT("Failed to create flipbook package: %s"), *PackagePath);
			continue;
		}
		...
		if (!NewFlipbook)
		{
			UE_LOG(LogTemp, Error, TEXT("Failed to create flipbook: %s"), *AnimationName);
			continue;
		}
		...
		UE_LOG(LogTemp, Warning, TEXT("✓ Saved flipbook package to disk: %s"), *PackagePath);
#endif
		
		CreatedAnimations.Add(NewFlipbook);
		
		UE_LOG(LogTemp, Log, TEXT("Created animation: %s with %d frames"), 
*AnimationName, NewFlipbook->GetNumKeyFrames());
	}

	UE_LOG(LogTemp, Log, TEXT("Created %d animations total"), CreatedAnimations.Num());
#endif
	...
	int32 StartX, int32 StartY, int32 SpriteWidth, int32 SpriteHeight, int32 BytesPerPixel, const FString& SpriteName)
{
	...
	if (!Package)
	{
		UE_LOG(LogTemp, Error, TEXT("Failed to create texture package: %s"), *PackageName);
		return nullptr;
	}
	...
	if (!NewTexture)
	{
		UE_LOG(LogTemp, Error, TEXT("Failed to create texture object: %s"), *AssetName);
		return nullptr;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("Created sprite texture: %s (%dx%d)"), *AssetName, SpriteWidth, SpriteHeight);
	return NewTexture;
#else
	UE_LOG(LogTemp, Error, TEXT("Texture creation is only available in editor builds"));
	return nullptr;
#endif
}